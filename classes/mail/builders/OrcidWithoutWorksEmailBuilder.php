...
class OrcidWithoutWorksEmailBuilder implements EmailBuilder
{
    private $endorsement;
    private $publication;
    ...
    public function build(array $args = []): Mailable
    {
        ...
        $email = new EndorserOrcidWithoutWorks($context, $submission, $this->emailParams);
        $email->from($context->getData('contactEmail'), $context->getData('contactName'));
        $email->to([['name' => $this->primaryAuthor->getFullName(), 'email' => $this->primaryAuthor->getEmail()]]);
